  import { ArrowBigRight, MinusIcon, PlusIcon, ShoppingBagIcon, Trash2Icon, XIcon } from "lucide-react";
      ...
      {/* overlay */}
        ...
        <div className="fixed right-0 top-0 h-full w-full max-w-md bg-white z-50 shadow-2xl flex flex-col animate-slide-in-right">
           ...
           <div className='flex-1 overflow-y-auto p-5 space-y-4 '>
            {items.length === 0 ?(
                <div className="flex flex-col items-center judtify-center justify-center h-full text-center ">
                  <ShoppingBagIcon className="size-10 text-app-border mb-4" />
                 ...
                 items.map((item)=>(
                  <div key={item.product._id} className="flex gap-3 bg-app-cream/60 rounded-xl p-3"> 
                    <img src={item.product.image} alt={item.product.name} className="rounded-lg size-16 object-cover shrink-0" />
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex justify-between items-center mt-2">
                        ...
                        <div className="flex items-center gap-1.5 ">
                          <button onClick={()=> updateQuantity(item.product._id, item.quantity - 1 )} className="rounded-lg size-7 bg-white border border-app-border flex-center ">
           ...
           {/* footer */}
            {items.length > 0 && (
               <div className="p-4 border-t border-app-border space-y-3">
                ...
                {deliveryFee > 0 && <p className="text-xs text-app-text-light text-light">Free delivery on order over {currency} 20!</p>}
                ...
                <button onClick={()=> {setIsCartOpen(false); navigate('/checkout'); window.scrollTo(0,0) }}
                 className="w-full py-3 mt-8 bg-orange-500 text-white font-semibold rounded-xl hover:bg-orange-dark transition-colors flex-center gap-2 active:scale-[0.98]">
                  Proceed to Checkout <ArrowBigRight className="size-4"/>
                </button>
